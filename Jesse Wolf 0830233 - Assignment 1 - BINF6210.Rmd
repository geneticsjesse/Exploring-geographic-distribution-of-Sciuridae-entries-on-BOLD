...
ggbar_nfilter<- ggplot(data = QC_data_count_nfilter, 
  ...
  coord_flip()+
  # Sorting the genus names in the x-axis alphabetically
  scale_x_discrete(limits = c(sort(x = unique(QC_data_count_nfilter$genus_name), decreasing = T)))

# Combining both plots using the patchwork package and 
# setting the title and subtitle themes
ggbar_combined <- ggbar_allgenera + inset_element(ggbar_nfilter, 0.1, 0.38, 1, 1) +
...
#### Figure 2: Each stacked bar represents a total count of individuals within a given genera that were submitted to BOLD (The Barcode of Life Data System) and is separated by colour, corresponding to a specific continent. Figure 2 depicts the number of BOLD entries of the *Sciuridae* genera that passed our data filtering and quality control (n=379). The inset figure visualizes the number of BOLD entries from a genera that had a sample size of greater than or equal to 5 (n=346), thus giving us a better idea of the dominant genera within our data. 
